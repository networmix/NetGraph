...
def test_notebook_export_creates_file(tmp_path: Path) -> None:
    ...
    step = NotebookExport(name="nb", output_path=str(output_file))
    step.run(scenario)
...
def test_notebook_export_empty_results_throws_exception(tmp_path: Path) -> None:
    ...
    step = NotebookExport(name="empty_nb", output_path=str(output_file))
...
def test_notebook_export_empty_results_with_allow_flag(tmp_path: Path) -> None:
    ...
    output_file = tmp_path / "empty.ipynb"
    step = NotebookExport(
        name="empty_nb", output_path=str(output_file), allow_empty_results=True
    )
    step.run(scenario)
...
def test_notebook_export_with_capacity_envelopes(tmp_path: Path) -> None:
    ...
    step = NotebookExport(name="env_nb", output_path=str(output_file))
    step.run(scenario)
...
def test_notebook_export_with_flow_data(tmp_path: Path) -> None:
    ...
    step = NotebookExport(name="flow_nb", output_path=str(output_file))
    step.run(scenario)
...
def test_notebook_export_mixed_data(tmp_path: Path) -> None:
    ...
    step = NotebookExport(name="mixed_nb", output_path=str(output_file))
    step.run(scenario)
...
def test_notebook_export_creates_output_directory(tmp_path: Path) -> None:
    ...
    step = NotebookExport(name="dir_test", output_path=str(output_file))
    step.run(scenario)
...
def test_notebook_export_large_dataset(tmp_path: Path) -> None:
    ...
    step = NotebookExport(name="large_nb", output_path=str(output_file))
    step.run(scenario)
...
def test_notebook_export_invalid_paths(bad_path: str) -> None:
    ...
    step = NotebookExport(name="bad_path", output_path=bad_path)
...
def test_notebook_export_serialization_error_handling(tmp_path: Path) -> None:
    ...
    step = NotebookExport(name="ser_nb", output_path=str(output_file))
