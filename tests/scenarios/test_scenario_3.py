...
def test_scenario_3_build_graph_and_capacity_probe() -> None:
    ...
    # For "my_clos1/b1/t1/t1-1", we expect hw_component="LeafHW-A" and risk_groups={"clos1-b1t1-SRG"}
    ...
    assert node_a1.risk_groups == {"clos1-b1t1-SRG"}, (
        "Expected risk_groups={'clos1-b1t1-SRG'} for 'my_clos1/b1/t1/t1-1'."
    )

    # For "my_clos2/b2/t1/t1-1", check hw_component="LeafHW-B" and risk_groups={"clos2-b2t1-SRG"}
    node_b2 = net.nodes["my_clos2/b2/t1/t1-1"]
    assert node_b2.attrs.get("hw_component") == "LeafHW-B"
    assert node_b2.risk_groups == {"clos2-b2t1-SRG"}

    # For "my_clos1/spine/t3-1", check hw_component="SpineHW" and risk_groups={"clos1-spine-SRG"}
    node_spine1 = net.nodes["my_clos1/spine/t3-1"]
    assert node_spine1.attrs.get("hw_component") == "SpineHW"
    assert node_spine1.risk_groups == {"clos1-spine-SRG"}
    ...
    # Another override sets risk_groups={"SpineSRG"} + hw_component="400G-LR4" on all spine-spine links
    ...
    for link_obj in link_id_2:
        assert link_obj.risk_groups == {"SpineSRG"}, (
            "Expected risk_groups={'SpineSRG'} on spine<->spine link."
        )
